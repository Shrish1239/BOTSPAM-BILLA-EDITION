...
def load_plugins(plugin_name):
    path = Path(f"CAT/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"CAT.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["CAT.modules." + plugin_name] = load
    print("CAT has Imported " + plugin_name)


files = glob.glob("CAT/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
...
print("\CAT bot 𝐃𝐞𝐩𝐥𝐨𝐲𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲")
